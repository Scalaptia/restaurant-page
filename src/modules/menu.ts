...
function getImgUrl(fileName: string) {
    const imgUrl = new URL(
        `../assets/menu-items/${fileName}.png`,
        import.meta.url
    ).href;
    return imgUrl;
}
...
export default function Menu() {
    ...
    warmMenu.appendChild(
        createItem("Hot Chocolate", 4, getImgUrl("hot_chocolate"))
    );
    ...
    coldMenu.appendChild(
        createItem("Mocha Frappe", 6.25, getImgUrl("mocha_frappe"))
    );
    coldMenu.appendChild(
        createItem("Caramel Frappe", 6.25, getImgUrl("caramel_frappe"))
    );
    ...
    foodMenu.appendChild(
        createItem("Snack Trays", 6, getImgUrl("snack_trays"))
    );
